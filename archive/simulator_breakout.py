"""Trade simulator — BREAKOUT variant of the MNQ ORB-cluster strategy.

Identical to simulator.py except direction is flipped in classify_setups:
for a cluster ABOVE price, BUY at first touch (cluster.low) expecting
continuation upward. For a cluster BELOW price, SELL at first touch
(cluster.high) expecting continuation downward. Same 30/30 stops, same
3pt cluster gap, same trading window, same one-position-at-a-time, same
stop-first conservative on same-bar ambiguity.

Output: results/archive/trades_breakout.parquet (does NOT touch the
locked baseline data/processed/trades.parquet).
"""
...
TRADES_PARQUET = ARCHIVE_DIR / "trades_breakout.parquet"
...
def classify_setups(clusters_today: list[Cluster], reference_price: float) -> list[Setup]:
    # First-touch BREAKOUT entry: enter at the near side of the cluster zone,
    # in the direction of the breakout (continuation through the zone).
    # Direction is flipped vs the fade simulator; entry price is the same.
    setups = []
    for c in clusters_today:
        if c.low > reference_price:
            # cluster ABOVE price: order fires when price RISES into cluster.low
            setups.append(Setup(side="buy", cluster=c, limit_price=c.low, trigger_above=True))
        elif c.high < reference_price:
            # cluster BELOW price: order fires when price FALLS into cluster.high
            setups.append(Setup(side="sell", cluster=c, limit_price=c.high, trigger_above=False))
...
def find_first_fill(setups: list[Setup], bar: dict) -> Setup | None:
    ...
    for s in setups:
        if s.triggered:
            continue
        # Trigger direction depends on where the cluster sits vs ref price,
        # NOT on the trade side (which is what differs between fade and breakout).
