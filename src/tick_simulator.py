...
from paths import TICKS_OVERLAP_PARQUET, ORB_TABLE_PARQUET, TRADES_PARQUET
...
def classify_setups(clusters_today, reference_price):
    ...
    for c in clusters_today:
        if c.low > reference_price:
            setups.append({"side": "sell", "limit_price": c.low,
                           "cluster": c, "triggered": False})
        elif c.high < reference_price:
            setups.append({"side": "buy", "limit_price": c.high,
                           "cluster": c, "triggered": False})
    return setups


def simulate_session(ts_ns, last_arr, setups, session_date,
                    window_start_ns, fc_utc_ns, stop_pts, target_pts):
    ...
    while i < fc_idx:
        ...
        for s in setups:
            ...
            if (best_idx is None) or (hit_i < best_idx) or (
                hit_i == best_idx and d < best_dist
            ):
                best_setup = s
                best_idx = hit_i
                best_dist = d
...
def run_config(stop_pts, target_pts, ticks_df, orb_table, overlap_dates):
    ...
    ticks_by_date = {d: g.sort_values("ts_utc").reset_index(drop=True)
                     for d, g in ticks_df.groupby("date")}
    ...
    for orb_row in orb_table.itertuples():
        ...
        if sd_date in overlap_dates:
            ...
            if session_ticks is not None and len(session_ticks):
                ts_ns = session_ticks["ts_utc"].astype("datetime64[ns, UTC]").astype("int64").to_numpy()
                last_arr = session_ticks["last"].to_numpy()
                window_start_ns = int(pd.Timestamp(f"{sd_date} 13:46:00",
                                                   tz="UTC").value)
                fc_utc_ns = int(pd.Timestamp(f"{sd_date} 15:30:00",
                                             tz="UTC").value)
                trades = simulate_session(ts_ns, last_arr, setups, sd,
                                          window_start_ns, fc_utc_ns,
                                          stop_pts, target_pts)
...
def report(trades, label):
    ...
    return {
        ...
        "wr": wr, "pts": pts, "usd": usd, "exp_pts": exp_pts,
        "max_dd": max_dd, "targets": targets, "stops": stops, "fc": fc,
...
def main() -> None:
    ...
    orb_table = pd.read_parquet(ORB_TABLE_PARQUET).sort_values("session_date").reset_index(drop=True)
    orb_table["session_date"] = pd.to_datetime(orb_table["session_date"])

    overlap_orb = orb_table[
        (orb_table["session_date"] >= OVERLAP_START)
        & (orb_table["session_date"] <= OVERLAP_END)
    ]
    ...
    bar_overlap = bar_trades[
        (bar_trades["session_date"] >= OVERLAP_START)
        & (bar_trades["session_date"] <= OVERLAP_END)
    ]
    n_b = len(bar_overlap)
    if n_b:
        ...
        peak = np.maximum.accumulate(eq); dd_b = float((eq - peak).min())
        print(f"\n=== BAR-BASED 1-MIN SIM (same overlap period) for Config A ===")
        print(f"  Trades:                   {n_b}")
        print(f"  Win rate:                 {wr_b:.1f}%")
        print(f"  Total P&L:                {bar_overlap['pnl_points'].sum():+.2f} pts  "
              f"(${bar_overlap['pnl_dollars'].sum():+,.2f})")
        print(f"  Expectancy/trade:         {bar_overlap['pnl_points'].mean():+.3f} pts")
        print(f"  Max drawdown:             ${dd_b:+,.2f}")
        print(f"  Targets / Stops / FC:     {er_b.get('target',0)} / {er_b.get('stop',0)} / {er_b.get('force_close',0)}")
        print(f"  Best / Worst trade:       ${bar_overlap['pnl_dollars'].max():+,.2f} / ${bar_overlap['pnl_dollars'].min():+,.2f}")
