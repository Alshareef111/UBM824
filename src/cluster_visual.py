...
def parse_session(arg, or_levels):
    ...
    if d not in or_levels.index:
        raise SystemExit(
            f"Session {d} not in OR levels; most recent is {or_levels.index[-1]}"
        )
    return d


def clusters_for(or_levels, session_date, gap, min_size,
    ...
    rows = [
        {"low": min(c), "high": max(c),
         "center": sum(c) / len(c), "n_levels": len(c)}
...
def compute_y_range(or_low, or_high, close, cluster_sets):
    ...
    nonempty = [cluster_sets[k][0]["center"] for k in cluster_sets
                if not cluster_sets[k][0].empty]
    if nonempty:
        all_centers = pd.concat(nonempty, ignore_index=True)
        nearest = all_centers.reindex(
            (all_centers - or_mid).abs().sort_values().index
        ).head(10)
...
def render_png(sess, pool_levels, cluster_sets, or_low, or_high, close):
    ...
    for lvl in pool_levels:
        ax.hlines(lvl, xmin=0.0, xmax=slot_w,
                  colors="lightgray", linewidth=0.6, alpha=0.7)
    ...
    for i, (key, gap, ms, color, lw, label) in enumerate(CONFIGS):
        ...
        for _, row in df.iterrows():
            ax.fill_between([x0, x1], row["low"], row["high"],
                            color=color, alpha=0.25)
            ax.hlines(row["center"], xmin=x0, xmax=x1,
                      colors=color, linewidth=lw)

    ax.axhline(or_high, color="black", linestyle="--", linewidth=1.1,
               alpha=0.8, label=f"OR high {or_high:.1f}")
    ax.axhline(or_low, color="black", linestyle="--", linewidth=1.1,
               alpha=0.8, label=f"OR low  {or_low:.1f}")
    if close is not None:
        ax.axhline(close, color="purple", linestyle="-", linewidth=1.3,
                   alpha=0.85, label=f"close   {close:.1f}")
    ...
    ax.set_ylabel("Price")
    ax.set_title(f"Cluster diagnostic  |  session {sess}\n"
...
def render_html(sess, pool_levels, cluster_sets, or_low, or_high, close):
    ...
    for key, gap, ms, mpl_color, lw, label in CONFIGS:
        ...
        for _, row in df.iterrows():
            ...
            band_y += [row["low"], row["low"], row["high"], row["high"],
                       row["low"], None]
        if band_x:
            fig.add_trace(go.Scatter(
                x=band_x, y=band_y,
        ...
        for _, row in df.iterrows():
            ...
            cd = [row["center"], int(row["n_levels"]),
    ...
    fig.update_layout(
        title=dict(
            text=(f"Cluster diagnostic  |  session {sess}<br>"
...
def main():
    ...
    if html_mode:
        out_path = render_html(sess, pool_levels, cluster_sets,
                               or_low, or_high, close)
        print_count_table(cluster_sets, or_low, or_high)
        print(f"\nHTML DONE  {out_path}")
    else:
        out_path = render_png(sess, pool_levels, cluster_sets,
                              or_low, or_high, close)
        print_count_table(cluster_sets, or_low, or_high)
