...
def compute_plus_minus_di(bars: pd.DataFrame, n: int = 15) -> pd.DataFrame:
    ...
    minus_di = 100.0 * minus_dm_s / tr_s.replace(0, np.nan)
    out = pd.DataFrame(
        {"plus_di": plus_di.to_numpy(), "minus_di": minus_di.to_numpy()}, index=bars.index
    )
    return out
...
def sanity_check():
    ...
    print(
        f"  ADX:      min={valid_adx.min():.3f}  max={valid_adx.max():.3f}   (must be in [0,100])",
        flush=True,
    )
    print(
        f"  +DI:      min={valid_pdi.min():.3f}  max={valid_pdi.max():.3f}   (must be in [0,100])",
        flush=True,
    )
    print(
        f"  -DI:      min={valid_mdi.min():.3f}  max={valid_mdi.max():.3f}   (must be in [0,100])",
        flush=True,
    )
