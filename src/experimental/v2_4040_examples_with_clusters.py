...
from experimental.v2_4040_examples import select_trades, format_text_panel
...
def reconstruct_cluster_pool(session_date, orb_table: pd.DataFrame) -> tuple[list, float, float, float]:
    ...
    return clusters, float(today_row["orb_close"]), float(today_row["orb_high"]), float(today_row["orb_low"])


def categorize_clusters(clusters: list, orb_close: float, traded_low: float, traded_high: float,
                        win_bars: pd.DataFrame) -> list[dict]:
    ...
    for c in clusters:
        low = float(c.low); high = float(c.high); size = int(c.size)
...
def session_window_bars(bars: pd.DataFrame, session_date) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    sd_date = sd.date()
    chart_start = pd.Timestamp.combine(sd_date, pd.Timestamp("09:00").time()).tz_localize("America/New_York")
    chart_end = pd.Timestamp.combine(sd_date, pd.Timestamp("11:45").time()).tz_localize("America/New_York")
    chart = day[(day["ts_ny"] >= chart_start) & (day["ts_ny"] <= chart_end)].reset_index(drop=True)

    trade_start = pd.Timestamp.combine(sd_date, pd.Timestamp("09:46").time()).tz_localize("America/New_York")
    trade_end = pd.Timestamp.combine(sd_date, pd.Timestamp("11:30").time()).tz_localize("America/New_York")
...
def draw_cluster_bands(ax, cluster_records: list[dict]):
    ...
    for fate in order:
        ...
        for c in by_fate[fate]:
            ax.axhspan(c["cluster_low"], c["cluster_high"],
                       facecolor=sty["color"], alpha=sty["alpha"], zorder=1)
            if sty["edge_lw"] > 0:
                ax.axhline(c["cluster_low"], color=sty["color"], alpha=sty["edge_alpha"],
                           linewidth=sty["edge_lw"], linestyle="-", zorder=1.5)
                ax.axhline(c["cluster_high"], color=sty["color"], alpha=sty["edge_alpha"],
                           linewidth=sty["edge_lw"], linestyle="-", zorder=1.5)
...
def landscape_summary(cluster_records: list[dict]) -> dict:
    ...
    return {"total": total, "above": above, "below": below, "spans": spans,
            "traded": traded, "touched": touched, "untouched": untouched}
...
def draw_one_trade(ax_chart, ax_text, trade: dict, chart_bars: pd.DataFrame,
                   cluster_records: list[dict], compact: bool = False):
    if chart_bars.empty:
    ...
    main_obj = ax_text.text(0.02, 0.99, txt, family="monospace", fontsize=fontsize,
    ...
    result_obj = ax_text.text(0.02, max(0.0, y_after_main - 0.015),
                               result_block.lstrip("\n"),
    ...
    ax_text.text(0.02, max(0.0, y_after_result - 0.015),
                 format_landscape_block(ls).lstrip("\n"),
                 family="monospace", fontsize=fontsize, va="top", ha="left",
                 transform=ax_text.transAxes)


def make_individual(trade: dict, idx: int, chart_bars: pd.DataFrame,
                    cluster_records: list[dict]) -> Path:
...
def write_manifest(rows: list[dict]):
    ...
    lines.append("Same 10 trades as `v2_4040_examples_20260515/`, now annotated with the full 9:45 cluster pool.")
    lines.append("")
    ...
    lines.append("- TOUCHED_NOT_TRADED: medium gray, near-border crossed by a bar's range during 09:46-11:30 but blocked by C2 or by classifier SKIP")
    ...
    lines.append("")
    lines.append("Y-axis on every figure is zoomed to entry ± 60 points, so the cluster density near the trade is visible. Clusters outside that band exist but are not visualized.")
    lines.append("")
    lines.append("| # | Date | Side | Label | Outcome | P&L | Total clusters | Above | Below | Spans | Touched-not-traded | Untouched | File |")
    lines.append("|---:|---|---|---|---|---:|---:|---:|---:|---:|---:|---:|---|")
    for r in rows:
        lines.append(
    ...
    for r in rows:
        ls = r["ls"]
        lines.append(f"{r['idx']}. **{r['date']} ({r['cat']})** — "
...
def main():
    ...
    print(f"Loaded {len(trades)} V2+40/40 trades, {len(bars):,} bars, {len(orb_table)} ORB sessions")
    ...
    for idx, trade in enumerate(selected, start=1):
        ...
        if traded_count != 1:
            print(f"  warning: session {sd.date()} has {traded_count} TRADED clusters "
        ...
        print(f"  wrote {fname.name}  (clusters: total {ls['total']}, above {ls['above']}, "
    ...
    print("\n" + "=" * 100)
    print(f"{'#':>3} {'date':<11} {'cat':<18} {'total':>6} {'above':>6} {'below':>6} {'spans':>6} {'touched-NT':>11} {'untouched':>10}")
    print("-" * 100)
    for r in rows:
        ls = r["ls"]
        print(f"{r['idx']:>3} {r['date']:<11} {r['cat']:<18} {ls['total']:>6} {ls['above']:>6} "
