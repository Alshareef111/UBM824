...
def write_report(
    ...
    var_b_stats: dict,
    var_b_wf: dict,
    var_b_yearly: pd.DataFrame,
    var_b_exits: dict,
    var_b_audit: dict,
    var_c_stats: dict,
    var_c_wf: dict,
    var_c_yearly: pd.DataFrame,
    var_c_exits: dict,
    var_c_audit: dict,
) -> None:
    ...
    lines.append(
        "Both variants use 2-contract sizing (p1 + p2), partial-exit at p1 = +40, "
    ...
    lines.append("")
    lines.append(
        "**Baseline column** = V2 + 40/40 (single-contract) numbers from "
    ...
    lines.append(
        "- Same-bar leg ordering: stop-first conservative applied **independently per "
    ...
    lines.append(
        "- BE-stop on the bar p1 exits (bar X): p2 still uses initial stop on bar X; "
        "BE-stop at entry becomes active on bar X+1."
    )
    lines.append(
        "- Entry-bar (i = bar of first fill): both legs evaluate exits on the entry bar "
        "itself (consistent with existing simulator_v2 behavior); the existing D-014 "
        "entry-bar chronology bias is inherited unchanged."
    )
    lines.append(
        "- If p1 hits +40 on the same bar p2 already exited (e.g., p2 cluster target "
        "closer than +40), no BE state is tracked for p2 (already closed)."
    )
    lines.append(
        "- If a session has NO clusters in trade direction at session open, p2 has "
    ...
    for vname, vstats, vwf, vyearly, vexits, vaudit, vfile, vparams in [
        ...
        lines.append("")
        lines.append(
            f"Median per-window OOS: {mdmoney(vwf['wf_median_oos'])} · "
        ...
        for yr in vyearly.index:
            r = vyearly.loc[yr]
            lines.append(
                f"| {yr} | {int(r['n_trades'])} | {mdmoney(r['pnl'])} | {mdpct(r['win_rate'])} |"
            )
        lines.append("")
        ...
        lines.append(
            f"- Runner-target source: {n_clu} entries had a cluster-derived target "
            f"({pct:.1%}); {n_no} entries had NO cluster in trade direction "
            f"(force-close fallback)."
        )
        lines.append(
            f"- Runner-BE fired (p1 hit +40, p2 still open after): {be_n} entries ({be_pct:.1%})."
        )
        lines.append(
            f"- p1 leg total: {mdmoney(vaudit['p1_pnl_total'])} · "
        ...
        lines.append("")
        lines.append(
            "- Entry cluster is excluded from runner-target candidates by Python "
        ...
        lines.append(
            "- Same-bar exit ordering: each leg evaluated independently using "
            "existing stop-first conservative (D-005). If bar range covers stop AND "
            "target for that leg, the leg stops."
        )
        lines.append(
            "- Entry bar can exit both legs at any combination (target/stop) using "
        ...
        lines.append(
            "- Runner-BE state is tracked in column `runner_be_fired`. Column "
    ...
    lines.append(
        "- `trades_variant_b_partial_runnerBE.parquet` — Variant B trade log "
        "(one row per entry, 2-contract aggregate P&L, runner-BE audit columns)"
    )
    lines.append(
        "- `trades_variant_c_tight_partial_runnerBE.parquet` — Variant C trade log "
        "(same schema, −25 stop)"
    )
    lines.append("")
    lines.append(
        "**No project files modified outside this directory and `src/experimental/`. "
...
def main() -> None:
    ...
    trades_b = sim_p.run_backtest(
        ...
        classifier,
        stop_points=40.0,
        target_p1_points=40.0,
    )
    df_b = sim_p.trades_to_dataframe(trades_b)
    print(
        f"  {len(df_b)} entries  total ${df_b['pnl_dollars'].sum():,.2f}  [{time.time() - t:.1f}s]",
        flush=True,
    )
    ...
    print(
        f"  {len(df_c)} entries  total ${df_c['pnl_dollars'].sum():,.2f}  [{time.time() - t:.1f}s]",
        flush=True,
    )
    ...
    all_dates = set(pd.to_datetime(df_b["session_date"])) | set(
        pd.to_datetime(df_c["session_date"])
    )
    ...
    rows = [
        (
            "Trades (entries)",
            f"{baseline['stats']['n_entries']:,}",
            f"{s_b['n_entries']:,}",
            f"{s_c['n_entries']:,}",
        ),
        (
            "WR%",
            fmt_pct(baseline["stats"]["win_rate"]),
            fmt_pct(s_b["win_rate"]),
            fmt_pct(s_c["win_rate"]),
        ),
        (
            "Total P&L",
            fmt_money(baseline["stats"]["total_pnl"]),
            fmt_money(s_b["total_pnl"]),
            fmt_money(s_c["total_pnl"]),
        ),
        (
            "Mean per entry",
            f"${baseline['stats']['mean_pnl']:.2f}",
            f"${s_b['mean_pnl']:.2f}",
            f"${s_c['mean_pnl']:.2f}",
        ),
        (
            "PF",
            f"{baseline['stats']['profit_factor']:.3f}",
            f"{s_b['profit_factor']:.3f}",
            f"{s_c['profit_factor']:.3f}",
        ),
        (
            "Max DD",
            fmt_money(baseline["stats"]["max_dd"]),
            fmt_money(s_b["max_dd"]),
            fmt_money(s_c["max_dd"]),
        ),
        (
            "DD duration (days)",
            f"{baseline['stats']['max_dd_duration_days']}",
            f"{s_b['max_dd_duration_days']}",
            f"{s_c['max_dd_duration_days']}",
        ),
        (
            "DD recovered",
            "yes" if baseline["stats"]["max_dd_recovered"] else "no",
            "yes" if s_b["max_dd_recovered"] else "no",
            "yes" if s_c["max_dd_recovered"] else "no",
        ),
        (
            "Ann. Sharpe",
            f"{baseline['stats']['sharpe_ann']:.2f}",
            f"{s_b['sharpe_ann']:.2f}",
            f"{s_c['sharpe_ann']:.2f}",
        ),
        (
            "Ann. Sortino",
            f"{baseline['stats']['sortino_ann']:.2f}",
            f"{s_b['sortino_ann']:.2f}",
            f"{s_c['sortino_ann']:.2f}",
        ),
        (
            "WF Sharpe-like",
            f"{baseline['wf']['wf_sharpe_like']:.2f}",
            f"{w_b['wf_sharpe_like']:.2f}",
            f"{w_c['wf_sharpe_like']:.2f}",
        ),
        (
            "Sign stability k/7",
            f"{baseline['wf']['wf_sign_count']}/7",
            f"{w_b['wf_sign_count']}/7",
            f"{w_c['wf_sign_count']}/7",
        ),
        (
            "Sum per-window OOS",
            fmt_money(baseline["wf"]["wf_oos_sum"]),
            fmt_money(w_b["wf_oos_sum"]),
            fmt_money(w_c["wf_oos_sum"]),
        ),
        (
            "4-gate qualify",
            "✓" if baseline["wf"]["wf_qualifies_deploy"] else "✗",
            "✓" if w_b["wf_qualifies_deploy"] else "✗",
            "✓" if w_c["wf_qualifies_deploy"] else "✗",
        ),
    ]
    header = (
        f"| {'Metric':<22} | {'Baseline ×2c':>16} | {'B: Partial':>16} | {'C: Tight+Partial':>18} |"
    )
    sep = f"|{'-' * 24}|{'-' * 18}|{'-' * 18}|{'-' * 20}|"
