...
def filter_effect_diagnostic(df_fb: pd.DataFrame, df_base: pd.DataFrame) -> dict:
    ...
    base_copy["_key"] = list(zip(
        base_copy["session_date"], base_copy["cluster_low"].round(4), base_copy["cluster_high"].round(4)
    ))
    ...
    new_fb_lookup["_key"] = list(zip(
        new_fb_lookup["session_date"], new_fb_lookup["cluster_low"].round(4), new_fb_lookup["cluster_high"].round(4)
    ))
    ...
    for key in shared_keys:
        ...
        if len(b) and len(f):
            shared_labels.append((
                b.iloc[0]["cluster_label"], f.iloc[0]["cluster_label"],
                b.iloc[0]["side"], f.iloc[0]["side"],
            ))
    ...
    return {
        ...
        "filtered_out_mean_pnl": float(filtered["pnl_dollars"].mean()) if len(filtered) else float("nan"),
        "filtered_out_wr": float((filtered["pnl_dollars"] > 0).mean()) if len(filtered) else float("nan"),
        ...
        "filtered_out_size_mean": float(filtered["cluster_size"].mean()) if len(filtered) else float("nan"),
        "filtered_out_size_median": float(filtered["cluster_size"].median()) if len(filtered) else float("nan"),
    }
...
def write_report(s_fb: dict, w_fb: dict, y_fb: pd.DataFrame, diag: dict) -> None:
    ...
    lines.append("")
    lines.append("**Change tested (only this):** limit price moved from the NEAR edge of each "
    ...
    lines.append("")
    lines.append("**Sub-classifier setups same as R-012:** ADX N=15 threshold=30, DI N=15 "
    ...
    lines.append("")
    lines.append(f"Median per-window OOS: {m(w_fb['wf_median_oos'])} · "
    ...
    lines.append(f"- Filtered out (baseline fires, far-border does not): **{diag['n_filtered_out']}**")
    lines.append(f"- New in far-border (far-border fires, baseline does not — C2 timing reorderings): **{diag['n_new_in_farborder']}**")
    lines.append("")
    ...
    lines.append(f"- Exit breakdown of filtered-out (in baseline): "
    ...
    fb_l = diag['fb_labels']
    lines.append(f"- Baseline:    FADE = {base_l.get('FADE', 0)}, TREND = {base_l.get('TREND', 0)} (total {sum(base_l.values())})")
    lines.append(f"- Far border:  FADE = {fb_l.get('FADE', 0)}, TREND = {fb_l.get('TREND', 0)} (total {sum(fb_l.values())})")
    lines.append("")
    lines.append("### Label flips on shared clusters:")
    lines.append("")
    lines.append(f"- Shared clusters where FADE/TREND label differs between baseline and far border: **{diag['n_label_flips_on_shared']}** of {diag['n_shared']}")
    lines.append(f"- Shared clusters where SIDE (buy/sell) differs (consequence of label flip): **{diag['n_side_flips_on_shared']}** of {diag['n_shared']}")
    lines.append("")
    lines.append("### Cluster-size distribution:")
    lines.append("")
    lines.append(f"- Baseline trades:        mean cluster_size = {diag['base_size_mean']:.2f}, median = {diag['base_size_median']:.1f}")
    lines.append(f"- Far-border trades:      mean cluster_size = {diag['fb_size_mean']:.2f}, median = {diag['fb_size_median']:.1f}")
    lines.append(f"- Filtered-out (baseline P&L): mean cluster_size = {diag['filtered_out_size_mean']:.2f}, median = {diag['filtered_out_size_median']:.1f}")
...
def main() -> None:
    ...
    print(f"  {len(df_fb)} trades  total ${df_fb['pnl_dollars'].sum():,.2f}  [{time.time()-t:.1f}s]", flush=True)

    df_fb.to_parquet(OUT_DIR / "trades_v2_4040_far_border.parquet", index=False)

    df_base = pd.read_parquet(BASELINE_PARQUET)

    all_dates = set(pd.to_datetime(df_fb["session_date"])) | set(pd.to_datetime(df_base["session_date"]))
    ...
    rows_terminal = [
        ...
        ("DD duration (days)",  f"{BASELINE_HEADLINE['max_dd_duration_days']}", f"{s_fb['max_dd_duration_days']}"),
        ("DD recovered (y/n)",  "y" if BASELINE_HEADLINE['max_dd_recovered'] else "n",
        ...
        ("WF Sharpe-like",      f"{BASELINE_HEADLINE['wf_sharpe_like']:.2f}", f"{w_fb['wf_sharpe_like']:.2f}"),
        ("Median per-window OOS", fm(BASELINE_HEADLINE['wf_median_oos']), fm(w_fb['wf_median_oos'])),
        ("Sign stability k/7",  f"{BASELINE_HEADLINE['wf_sign_count']}/7", f"{w_fb['wf_sign_count']}/7"),
        ("Sum per-window OOS",  fm(BASELINE_HEADLINE['wf_oos_sum']),       fm(w_fb['wf_oos_sum'])),
        ("4-gate qualified",    "✓" if BASELINE_HEADLINE['wf_qualifies_deploy'] else "✗",
    ...
    print(f"Filtered-out exit breakdown:       target={diag['filtered_out_n_target']}  stop={diag['filtered_out_n_stop']}  fc={diag['filtered_out_n_force_close']}")
    print()
    print(f"Label split — Baseline:    FADE={diag['base_labels'].get('FADE',0)}  TREND={diag['base_labels'].get('TREND',0)}")
    print(f"Label split — Far border:  FADE={diag['fb_labels'].get('FADE',0)}  TREND={diag['fb_labels'].get('TREND',0)}")
    print(f"Shared clusters with label flip:   {diag['n_label_flips_on_shared']}/{diag['n_shared']}")
    print(f"Shared clusters with side flip:    {diag['n_side_flips_on_shared']}/{diag['n_shared']}")
    print()
    print(f"Cluster size — Baseline trades:    mean={diag['base_size_mean']:.2f}  median={diag['base_size_median']:.1f}")
    print(f"Cluster size — Far-border trades:  mean={diag['fb_size_mean']:.2f}  median={diag['fb_size_median']:.1f}")
    print(f"Cluster size — Filtered-out:       mean={diag['filtered_out_size_mean']:.2f}  median={diag['filtered_out_size_median']:.1f}")
