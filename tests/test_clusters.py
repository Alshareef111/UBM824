...
def _check(label, result, expected_clusters):
    ...
    assert len(result) == len(expected_clusters), (
        f"{label}: expected {len(expected_clusters)} cluster(s), "
    ...
    for i, (c, (lo, hi, levels)) in enumerate(zip(result, expected_clusters)):
        ...
        assert c.levels == levels, (
            f"{label}[{i}].levels: expected {levels}, got {c.levels}"
        )
        assert c.size == len(levels), (
            f"{label}[{i}].size: expected {len(levels)}, got {c.size}"
        )
    print(f"  OK  {label}")
...
def test_single_chain_within_gap():
    ...
    _check("single chain of 4 within gap",
           r, [(10.0, 13.0, (10.0, 11.0, 12.0, 13.0))])
...
def test_chain_breaks_at_gap_boundary():
    ...
    r = find_clusters([100.0, 102.0, 105.0, 109.0],
                      max_gap=3.0, min_size=3)
    _check("chain breaks past gap, singleton dropped",
           r, [(100.0, 105.0, (100.0, 102.0, 105.0))])
...
def test_two_clusters_separated_by_wide_gap():
    ...
    r = find_clusters([10.0, 11.0, 12.0, 100.0, 101.0, 102.0],
                      max_gap=3.0, min_size=3)
    _check("two clusters split by wide gap",
           r, [
...
def test_unsorted_input_is_sorted_internally():
    ...
    _check("unsorted input sorted internally",
           r, [(10.0, 12.0, (10.0, 11.0, 12.0))])
...
def test_boundary_gap_equals_max_gap_inclusive():
    ...
    _check("gap == max_gap counts (inclusive)",
           r, [(100.0, 106.0, (100.0, 103.0, 106.0))])


def test_min_size_two_allows_pairs():
    # With min_size=2 we keep adjacent pairs that survived the gap rule.
    r = find_clusters([10.0, 11.0, 100.0, 101.0],
                      max_gap=3.0, min_size=2)
    _check("min_size=2 keeps pair clusters",
           r, [
               (10.0,  11.0,  (10.0, 11.0)),
...
def test_duplicate_levels_preserved():
    ...
    _check("duplicates preserved (no dedupe)",
           r, [(10.0, 11.0, (10.0, 10.0, 11.0))])
